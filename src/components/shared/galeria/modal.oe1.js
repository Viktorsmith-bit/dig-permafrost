...
import { listaOE1} from './listaJson/lista.galeria';
...
export default function Modal(props){
    ...
    const back = (e)=>{
        e.preventDefault();
        if(state < listaOE1.length){
            return setState(state + 1);
        }
    }
    const next = (e)=>{
        e.preventDefault();
        if(state > listaOE1[0].id){
            return setState(state - 1)
        }
    }
    const disable = ()=>{
                            ...
                            {listaOE1.map((e) =>{
                                if(e.id === state){
                                    return <div className='flex flex-col lg:flex-row h-screen lg:h-auto'>
                                        ...
                                        <div className='flex flex-col justify-between w-full lg:w-80 bg-gray-100 p-4 h-full lg:h-auto'>
                                            ...
                                            <h1 className='text-xs text-justify text-color text-color'>©2022 Permafrost. Todos los derechos reservados. Para obtener la imagen en calidad original ponerse en contacto al correo katydamacia@gmail.com.</h1>
                                        </div>
                                </div>
                                }
                            }) 
